...
Reader.prototype.load = function(filepath) {
	...
	if (typeof this.config.poster[type] != undefined ) {
	    var t_opts = this.config.target[this.config.poster[type]];
	    console.log('using target config: ', t_opts);

	    if (t_opts) {
		this.poster = new Poster(t_opts.url);
